...
ol.layer.Base = function(options) {
  ...
  properties[ol.layer.LayerProperty.OBLIQUE_INCLINATION] =
      goog.isDef(options.obliqueInclination) ? options.obliqueInclination : 90.0;

  properties[ol.layer.LayerProperty.COLOR_SCALE] =
      goog.isDef(options.colorScale) ? options.colorScale : [0.0,1.0];

  properties[ol.layer.LayerProperty.COLOR_RAMP] =
      goog.isDef(options.colorRamp) ? options.colorRamp : {};

  properties[ol.layer.LayerProperty.WATER_BODIES] =
      goog.isDef(options.waterBodies) ? options.waterBodies : true;

  properties[ol.layer.LayerProperty.LIGHT_ZENITH] =
      goog.isDef(options.lightZenith) ? options.lightZenith : 45;

  properties[ol.layer.LayerProperty.LIGHT_AZIMUTH] =
      goog.isDef(options.lightAzimuth) ? options.lightAzimuth : 225;

 properties[ol.layer.LayerProperty.AMBIENT_LIGHT] =
      goog.isDef(options.ambientLight) ? options.ambientLight : 0.0;


  properties[ol.layer.LayerProperty.TESTING] =
      goog.isDef(options.testing) ? options.testing : false;

  properties[ol.layer.LayerProperty.RESOLUTION] =
      goog.isDef(options.resolution) ? options.resolution : 1;

  properties[ol.layer.LayerProperty.HILL_SHADING] =
      goog.isDef(options.hillShading) ? options.hillShading : true;
